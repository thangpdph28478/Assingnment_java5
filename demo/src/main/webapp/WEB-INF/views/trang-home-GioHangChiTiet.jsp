...
<table class="table table-bordered border-primary">
    ...
    <c:forEach items="${dulieu}" var="ht" varStatus="">
        <tr>
            ...
            <th>${ht.id.idChiTietSP.idSp.ten} </th>
            <th>${ht.soluong}</th>
            <td>${ht.donGia}</td>
            <td>${ht.donGiaKhiGiam}</td>


            <td><a href="/gio-hang-chi-tiet/remove/${ht.id.idGioHang.id}/${ht.id.idChiTietSP.id}"
                   onclick="return myFunction1()"
                   class="btn btn-success">DELETE</a></td>


            <td><a href="/gio-hang-chi-tiet/view-update/${ht.id.idGioHang.id}/${ht.id.idChiTietSP.id}"
